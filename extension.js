...
const config = {
    ...
    settings: [
        ...
        },
        {
            ...
            description: "Comma-separated list of screening services NOT to list results for",
...
async function getAvailability({ extensionAPI }, parentOnly) {
    var rAPIkey, rAPIcc, omdbKey, exclude, excludeList, excludeItems, key;
    ...
    breakme: {
        if (!extensionAPI.settings.get("sa-rAPI-key")) {
            ...
            if (extensionAPI.settings.get("sa-exclude") != null) {
                exclude = extensionAPI.settings.get("sa-exclude");
                excludeList = exclude.split(",");
                excludeItems = excludeList.map(element => {
                    return element.trim().toLowerCase();
                });
                excludeList = excludeItems.join(':');
            }
        }
        ...
        if (parentOnly) { // get data for one film only
            ...
            if (imdbID == null) {
                ...
                if (!imdbID) {
                    alert("No results found at IMDb for this title, or you cancelled the search");
                    return;
                }
                ...
                .then(response => {
                    if (Object.keys(response.streamingInfo).length > 0) {
                        for (var i = 0; i < Object.keys(response.streamingInfo).length; i++) {
                            ...
                            if (excludeList.indexOf(name.toLowerCase()) === -1) {
